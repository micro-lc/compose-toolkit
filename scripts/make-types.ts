...
const main = async () => {
  ...
  try {
          ...
          http: {
            read({ url }) {
              const fileNameMatch = url.match(
                /micro-lc\/compose-toolkit\/main\/schemas\/(.+)(#\/.*)?$/
              )

              if (
                fileNameMatch?.[1] !== undefined
                  && fs.existsSync(path.resolve(schemaDirPath, fileNameMatch[1]))
              ) {
                return fs.readFileSync(
                  path.resolve(schemaDirPath, fileNameMatch[1])
                )
              }
    ...
    await fs.promises.writeFile(outputFilePath, compiledTypes)
